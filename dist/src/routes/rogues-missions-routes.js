...
const tgDataRequest = zod_1.z.object({
    ...
    mission_id: zod_1.z.string(),
});
async function routes(fastify, options) {
    fastify.get("/api/rogues_missions/:mission_id", async (request, reply) => {
    ...
    fastify.get("/api/rogues_missions_users/:user_id", async (request, reply) => {
        if (!fastify.mongo || !fastify.mongo.db)
            throw new Error("MongoDB is not configured properly");
    ...
    fastify.post("/api/rogues_missions_users", async (request, reply) => {
        try {
    ...
    async function doStatusUpdates(fastify, user_id, status) {
        ...
        if (not_items) {
            for (const not_item of not_items) {
                ...
                const hasItem = await (0, db_1.hasItemsUsers)(fastify, user_id, item_id, 1);
                if (hasItem)
                    return false;
            }
        }
        if (remove_items) {
            for (const remove_item of remove_items) {
                ...
                const hasItem = await (0, db_1.hasItemsUsers)(fastify, user_id, item_id, quantity);
                if (!hasItem)
                    return false;
            }
            for (const remove_item of remove_items) {
                const { item_id, quantity } = remove_item;
                await (0, db_1.removeItemsUsers)(fastify, user_id, item_id, quantity);
            }
        }
        if (has_items) {
        ...
        if (add_items) {
            for (const add_item of add_items) {
                ...
                await (0, db_1.addItemsUsers)(fastify, user_id, item_id, quantity);
            }
        }
        return true;
    }
    async function updateMissionToMissionUser(fastify, user_id, mission_id) {
        if (!fastify.mongo || !fastify.mongo.db)
            throw new Error("MongoDB is not configured properly");
        //Look up the mission
